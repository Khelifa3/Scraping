...
# Main scraping
file_name = "goldenpagesHealth.xlsx"
wb_obj, sheet_obj = process_excel.openExcel(file_name)
sheet_obj.title = "Health"
...
def scrapeLink(url):
    ...
    # Extract email
    try:
        email = re.findall(email_pattern, str(soup))[0].strip()
    except:
        return
...
def scrape_schools(page):
    url = f"https://www.goldenpages.ie/q/business/advanced/what/Health+Centres/{page}"
    response = makeRequest(url)
    soup = BeautifulSoup(response.text, "html.parser")
