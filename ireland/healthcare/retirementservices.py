...
def scrape_link(link):
    ...
    for item in items:
        ...
        email = re.findall(email_pattern, address_tag.text.strip())
        if len(email) > 0:
            email = email[0]
        else:
...
def scrape_entities():
    url = f"https://retirementservices.ie/nursing-homes-ireland/"
    try:
        response = scraper.get(url)
    except Exception as e:
        print(e)
    ...
    links = [
        ...
        if a["href"].startswith("https://retirementservices.ie/nursing-homes-")
    ]
    items = []
    for link in links:
